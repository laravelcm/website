<header class="flex flex-shrink-0 bg-white border-t-4 border-brand-primary shadow-smooth h-18">
    <button @click.stop="sidebarOpen = true" class="px-6 border-r border-gray-200 text-brand-primary hover:text-brand-900 focus:outline-none focus:bg-gray-100 focus:text-gray-600 md:hidden" aria-label="Open sidebar">
        <svg class="h-7 w-7" stroke="currentColor" fill="none" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" />
        </svg>
    </button>
    <div class="flex-shrink-0 hidden md:flex px-4 py-3 w-72">
        <a href="{{ route(home_route()) }}" class="inline-flex items-center w-full text-brand-primary flex-grow">
    ...
    <div class="flex-1 flex items-center justify-between px-6 lg:px-8">
        ...
        <div class="ml-4 flex items-center space-x-2">
            ...
            <div @click.away="open = false" class="relative hidden md:block" x-data="{ open: false }">
                <button @click="open = !open" class="flex flex-shrink-0 h-7 w-7 text-sm rounded-full text-white focus:outline-none">
                    <img class="h-full w-full rounded-full" src="{{ $logged_in_user->picture }}" alt="{{ $logged_in_user->email }}" />
                </button>
                <div x-show="open"
                     ...
                     class="origin-top-right absolute z-60 right-0 mt-2 w-60 bg-white rounded-md shadow-lg"
                >
                    <div class="flex items-center bg-gray-50 px-3 py-4">
                        <div class="flex flex-shrink-0 h-15 w-15 rounded-sm overflow-hidden">
                            <img src="{{ $logged_in_user->picture }}" alt="{{ $logged_in_user->email }}" class="h-full w-full object-cover"/>
                        </div>
                        <div class="pl-4 truncate">
                            <span class="block text-gray-800 font-medium truncate">{{ $logged_in_user->full_name }}</span>
