...
@section('meta')
    <meta name="description" content="{{ str_limit($package->resume, 150) }}">
    <!-- Facebook Meta -->
    <meta property="og:url" content="{{ route('packages.post', ['slug' => $package->slug]) }}">
    <meta property="og:title" content="{{ $package->title }} - Laravel Cameroon">
    <meta property="og:image" content="{{ asset("storage/$package->image") }}">
    <meta property="og:description" content="{{ str_limit($package->resume, 150) }}">
    <!-- Twitter Meta -->
    <meta name="twitter:url" content="{{ route('packages.post', ['slug' => $package->slug]) }}">
    <meta name="twitter:title" content="{{ $package->title }} - Laravel Cameroon">
    <meta name="twitter:description" content="{{ str_limit($package->resume, 150) }}">
    <meta name="twitter:image" content="{{ asset("storage/$package->image") }}">
@endsection
    ...
    <div class="container">
        ...
        <article class="single-post">
            ...
            <header class="post__header">
                <span class="post__date">{{ $package->created_at->format('M d, Y') }}</span>
                <h1 class="post__title">{{ $package->title }}</h1>
            ...
            <div class="post__content">
                <?= \DevDojo\Chatter\Helpers\ChatterHelper::demoteHtmlHeaderTags( GrahamCampbell\Markdown\Facades\Markdown::convertToHtml( $package->content ) ); ?>
            </div>
