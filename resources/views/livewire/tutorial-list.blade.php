    ...
    <div class="bg-white overflow-hidden shadow rounded-md">
        ...
        @if($tutorials->total() > 0)
                            ...
                            <tbody class="bg-white">
                                @foreach($tutorials as $tutorial)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-no-wrap border-t border-gray-100">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    @if($tutorial->image)
                                                        <img class="h-10 w-10 object-cover rounded-md" src="{{ $tutorial->image }}" alt="{{ $tutorial->title }}" />
                                                    @else
                                                ...
                                                <div class="ml-4 truncate">
                                                    <h3 class="text-sm leading-5 font-medium text-gray-900 truncate">{{ $tutorial->title }}</h3>
                                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $tutorial->status_classname }}">
                                                        {{ $tutorial->getStatus() }}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap border-t border-gray-100">
                                            <div class="text-sm leading-5 text-gray-900">{{ $tutorial->category->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap border-t border-gray-100">
                                            <span class="inline-flex capitalize items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-red-100 text-red-800">
                                              {{ $tutorial->provider }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap border-t border-gray-100 text-sm leading-5 text-gray-500">
                                            {{ $tutorial->published_at->format('d F, Y') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap border-t border-gray-100 text-sm leading-5 text-gray-500">
                                            <div class="text-sm leading-5 text-gray-900">{{ $tutorial->user->full_name }}</div>
                                            <div class="text-xs leading-5 text-gray-500">{{ $tutorial->user->rolesLabel }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap text-right border-t border-gray-100 text-gray-500 text-sm leading-5 font-medium">
                                            <div class="flex items-center space-x-2">
                                                <a href="{{ route('admin.tutorials.edit', $tutorial) }}" class="text-brand-200 hover:text-brand-900 focus:outline-none">
                                                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-5 h-5">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                    </svg>
                                                </a>
                                                <button wire:click="delete({{ $tutorial->id }})" type="button" class="text-brand-200 hover:text-brand-900 focus:outline-none">
            ...
            <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                <div class="flex-1 flex justify-between sm:hidden">
                    {{ $tutorials->links('components.wire-mobile-pagination-links') }}
                </div>
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm leading-5 text-gray-700">
                            {{ __('Affichage') }}
                            <span class="font-medium">{{ ($tutorials->currentPage() - 1) * $tutorials->perPage() + 1 }}</span>
                            {{ __('de') }}
                            <span class="font-medium">{{ ($tutorials->currentPage() - 1) * $tutorials->perPage() + count($tutorials->items()) }}</span>
                            {{ __('sur') }}
                            <span class="font-medium"> {!! $tutorials->total() !!}</span>
                            {{ __('résultats') }}
                        </p>
                    </div>
                    {{ $tutorials->links() }}
        ...
        @else
            <div class="flex flex-col items-center justify-center px-4 sm:px-6 py-6">
                ...
                <h3 class="text-gray-600 font-medium text-base leading-5 mt-2">{{ __("Aucun tutoriel.") }}</h3>
            </div>
        @endif
    </div>
</div>
