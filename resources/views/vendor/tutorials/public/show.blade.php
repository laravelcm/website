...
@section('title', $model->title.' – '.__('Tutorials').' – '.$websiteTitle)
@section('ogTitle', $model->title)
@section('description', $model->summary)
@section('image', $model->present()->thumbUrl())
@section('bodyClass', 'body-tutorials body-tutorial-'.$model->id.' body-page body-page-'.$page->id)
...
@section('content')
    ...
    <div class="container">
        ...
        <article class="single-post" itemscope itemtype="http://schema.org/Article">
            ...
            <header class="post__header">
                <span class="post__date">{{ __('Published on') }}
                    <time class="news-date" itemprop="datePublished" datetime="{{ $model->created_at->toIso8601String() }}">{{ $model->present()->dateLocalized('created_at') }}</time>
                </span>
                <h1 class="post__title" itemprop="name">{{ $model->title }}</h1>
                <ul class="post__metadata">
                    <li class="author">
                        {{ __('by') }} <strong class="text-primary">{{ $model->user->getFullName() }}</strong>
                    </li>
                    <li class="views">
                        <i class="fa fa-eye"></i> {{ $model->getViews() }} {{ __('views') }}
                    </li>
                </ul>
            </header>
            <div class="post__content" itemprop="articleBody">
                <?= \DevDojo\Chatter\Helpers\ChatterHelper::demoteHtmlHeaderTags( GrahamCampbell\Markdown\Facades\Markdown::convertToHtml( $model->present()->body ) ); ?>
            </div>
            <div class="post__footer">
                <p>
                    <span>{{ __('Share') }}</span>
                    <a href="javascript:;" data-url="{{ URL::full() }}" class="share_facebook">
                    ...
                    <a href="javascript:;" data-url="{{ URL::full() }}" class="share_twitter">
                        <svg width="35" height="35" viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="post_pagination">
        <div class="container">
            @if(!is_null($prevPost))
                <a href="{{ $prevPost->uri() }}" class="pagination__left">
                    ...
                    <div class="pagination__content">
                        ...
                        <p>{{ str_limit($prevPost->summary, 110) }}</p>
                    </div>
                </a>
            @endif
            @if(!is_null($nextPost))
                <a href="{{ $nextPost->uri() }}" class="pagination__right">
                    ...
                    <div class="pagination__content">
                        ...
                        <p>{{ str_limit($nextPost->summary, 110) }}</p>
                    </div>
                </a>
            @endif
        </div>
