...
@section('title')
	@if((Request::is('fr/' . Config::get('chatter.routes.home') . '/*' ) && isset($discussion->title)) || (Request::is('en/' . Config::get('chatter.routes.home') . '/*' ) && isset($discussion->title)) || (Request::is(Config::get('chatter.routes.home') . '/*' ) && isset($discussion->title)))
		{{ $discussion->title }}
	@endif
@stop
            ...
            @endif
	                	...
	                	@foreach($posts as $post)
	                		<li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
		                		<span class="chatter_posts">
		                			@if(!Auth::guest() && (Auth::user()->id == $post->user->id || Auth::user()->hasRole('administrator')))
				...
				@else
					...
					<div id="login_or_register">
						<p>
							{{ __("S'il vous plait") }} <a href="{{ route('login') }}">{{ __('login') }}</a> {{ __('or') }} <a href="{{ route('register') }}">{{ __('register') }}</a> {{ __('to leave a response') }}.
                        </p>
					</div>
