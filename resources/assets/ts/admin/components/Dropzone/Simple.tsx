...
const DropzoneSimple = () => {
  ...
  const dragClass = classNames('flex justify-center pt-5 pb-6 px-6 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-brand-primary hover:bg-gray-50', {
    'hover:border-brand-200 hover:bg-gray-50': isDragActive || isDragAccept,
  ...
  function removeFile() {
    axios.delete(`/remove-file/${value}`)
      .then(() => {
        setFile(null);
        setShowFile(false);
        setPreview('');
        ...
        <div {...getRootProps({ className: dragClass })}>
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-600">
              <button
                type="button"
                className="font-medium text-brand-primary hover:text-brand-200 focus:outline-none focus:underline transition duration-150 ease-in-out"
              >
                Upload
              </button>
              {" "}
