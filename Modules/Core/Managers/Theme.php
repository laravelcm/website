...
class Theme implements ThemeContract
{
    ...
    public function __construct(Container $app, ViewFinderInterface $finder, Repository $config, Translator $lang)
    {
        ...
        $this->basePath = $this->config['core.theme_path'];
     ...
     * @return null|mixed
     */
    public function getmixed($themeName)
    {
    ...
    {
        return !$collection ? $this->activeTheme : $this->getmixed($this->activeTheme);
    }
    ...
    public function getFullPath($path)
    {
        ...
        $themeInfo = $this->getmixed($themeName);

        if ( $this->config[ 'core.symlink' ] ) {
            $themePath = 'Themes/' . $themeName . '/';
        } else {
            $themePath = str_replace(base_path('public') . '/', '', $themeInfo->get('path')) . '/';
        }

        $assetPath = $this->config['core.folders.assets'].'/';
        $fullPath = $themePath.$assetPath.$path;

        if (!file_exists($fullPath) && $themeInfo->has('parent') && !empty($themeInfo->get('parent'))) {
            $themePath = str_replace(base_path().'/', '', $this->getmixed($themeInfo->get('parent'))->get('path') ).'/';
            $fullPath = $themePath.$assetPath.$path;
    ...
    public function lang($fallback)
    {
        ...
        if (count($splitLang) > 1) {
            ...
            if (!$this->lang->has($fallback)) {
                $fallback = $this->getmixed($this->current())->get('parent').'::'.$splitLang[0];
            }
        }
    ...
    private function scanThemes()
    {
        ...
        foreach ($themeDirectories as $themePath) {
            $themeConfigPath = $themePath.'/'.$this->config['core.config.name'];
            $themeChangelogPath = $themePath.'/'.$this->config['core.config.changelog'];
    ...
    private function loadTheme($theme)
    {
        ...
        $viewPath = $themeInfo->get('path').'/'.$this->config['core.folders.views'];
        $langPath = $themeInfo->get('path').'/'.$this->config['core.folders.lang'];
