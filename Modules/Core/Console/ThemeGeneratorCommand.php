...
class ThemeGeneratorCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'lb-theme:create {name?}';
    ...
    {
        $this->themePath = $this->config->get('core.theme_path');
    ...
    protected function init()
    {
        ...
        $this->themeFolders = $this->config->get('core.folders');
        $this->themeStubPath = $this->config->get('core.stubs.path');

        $themeStubFiles = $this->config->get('core.stubs.files');
        $themeStubFiles['theme'] = $this->config->get('core.config.name');
        $themeStubFiles['changelog'] = $this->config->get('core.config.changelog');
        ...
        foreach ($themeStubFiles as $filename => $storePath) {
            if ($filename === 'changelog') {
                ...
                $this->theme[$filename] = ltrim($storePath,
                    rtrim($this->config->get('core.folders.assets'), '/').'/');
            }
