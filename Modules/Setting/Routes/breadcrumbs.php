...
Breadcrumbs::for('admin.setting.index', function ($trail) {
    ...
    $trail->push(__('setting::menus.backend.title'), route('admin.setting.index'));
});

Breadcrumbs::for('admin.setting.env', function ($trail) {
    $trail->parent('admin.setting.index');
    $trail->push(__('setting::menus.backend.env_title'), route('admin.setting.env'));
});
